...
const { getNumber, checkSMS, waitForSMS, cancelNumber } = require('./sms.js');
...
async function handlePhoneVerification(page, accountData) {
  try {
    ...
    let phoneDetails;
    try {
      console.log("Getting phone number from SMS service...");
      phoneDetails = await getNumber(); // Assuming service ID 1 for Gmail and UK country
      console.log(`Received phone number: ${phoneDetails.number}`);
    } catch (error) {
    ...
    // Clear any existing text in the phone field with Ctrl+Backspace
    await clickHumanLike(page, 'input[type="tel"]');
    await pageWaitForTimeout(300 + Math.floor(Math.random() * 500));
    await typeHumanLike(page, 'input[type="tel"]', '+44');
    ...
    /*@ MOBILE VERIFICATION STARTS */
    // Type the phone number with human-like behavior

    await typeHumanLike(page, 'input[type="tel"]', phoneDetails.number);
    await pageWaitForTimeout(700 + Math.floor(Math.random() * 1000));
    ...
    if (nextButton) {
      ...
      await cancelNumber(phoneDetails.id); // Cancel the number request if we can't proceed
      return;
    }
    ...
    await page.waitForSelector('input[type="tel"]', { visible: true, timeout: 120000 });
    
    // Wait for the SMS to arrive (up to 2 minutes)
    const verificationCode = await waitForSMS(phoneDetails.id, 120000, 5000);
    console.log("verification code " , verificationCode);
    
    if (!verificationCode) {
      console.error("Failed to receive SMS verification code within the timeout period");
      await cancelNumber(phoneDetails.id);
      return;
    }
    ...
    await typeHumanLike(page, 'input[type="tel"]', verificationCode);
    await pageWaitForTimeout(500 + Math.floor(Math.random() * 800));
    

    // Look for a skip button with various selectors
    const skipButton = await page.$('button[type="button"]::-p-text(skip)') || 
               await page.$('button[type="button"]::-p-text(Skip)') ||
               await page.$('button:contains("skip")') ||
               await page.$('button:contains("Skip")');
    ...
    if (skipButton) {
      ...
      await pageWaitForTimeout(500 + Math.floor(Math.random() * 800));
      await clickHumanLike(page, skipButton);
      
      // Wait briefly after clicking skip
      await pageWaitForTimeout(1000 + Math.floor(Math.random() * 1500));
      
      console.log("Skipped verification code entry");
    ...
    await page.waitForSelector("button[type='button']");
      ...
      // Wait for 'I agree' button and click with human-like behavior
      ...
      try {
        ...
        await page.waitForFunction(() => {
          ...
          return buttons.some(button => 
            button.innerText.toLowerCase().includes('i agree') || 
        ...
        if (agreeButton) {
          console.log("'I agree' button found, clicking...");
          await performRandomMouseMovements(page);
          await pageWaitForTimeout(700 + Math.floor(Math.random() * 1000));
          await clickHumanLike(page, agreeButton);
        ...
        console.error("Error waiting for or clicking 'I agree' button:", error.message);
      }



    console.log("No skip button found, proceeding with verification code entry");
    // Click verify button
    const verifyButton =await page.$('button[type="button"]');
    if (verifyButton) {
